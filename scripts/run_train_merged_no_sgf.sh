...
if [[ -x "$VENV_AC" ]]; then
  exec "$VENV_AC" train-merged --y-axis 1 --exclude-symbolic --model-basename bayesian_cost_merged_no_sgf "$@"
fi
if [[ -x "$VENV_PY" ]]; then
  exec "$VENV_PY" -m autoconfig.cli train-merged --y-axis 1 --exclude-symbolic --model-basename bayesian_cost_merged_no_sgf "$@"
fi
